...
\begin{verbatim}
  ...
  [Client Module]                                   [Client Module]
         |                                                 |
         | (sends to own relay)                           | (sends to own relay)
         v                                                 v
  [Relay Server]                                    [Relay Server]
    Port: 7001                                        Port: 7002
         |                                                 |
         | (forwards to peer)                             | (forwards to peer)
         v                                                 v
         |                                                 |
         +-----------------(network)---------------------->|
         |                                                 |
  ...
  Message Path Example (A -> B):
  --------------------------------
  1. User types in Node A terminal
  2. Client Module sends to localhost:7001 (own Relay Server)
  3. Relay Server forwards to Node B's External Server (port 6002)
  4. Node B's External Server receives and displays message

  Reply Path (B -> A): Exact reverse through Node B's relay to Node A's external server
\end{verbatim}
